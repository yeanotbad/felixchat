...
function notifyOnlineStatus(uid) {
  const user = db.users[uid];
  if (!user) return;
  const online = sockets.has(uid) && sockets.get(uid).size > 0;
  for (const friendId of (user.friends || [])) {
    broadcast(friendId, { type: "online_status", uid, online });
  }
}
...
wss.on("connection", ws => {
  ...
  ws.on("message", raw => {
    try {
      ...
      if (message.type === "screenshot") {
        if (uid && message.to && areFriends(uid, message.to)) {
          broadcast(message.to, { type: "screenshot", from: uid, area: message.area || "chat" });
        }
        return;
      }
  ...
  ws.on("close", () => {
    ...
    if (!set) return;
    set.delete(ws);
    if (!set.size) { sockets.delete(uid); notifyOnlineStatus(uid); }
  });
});
